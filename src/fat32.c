...
void fat32_free_chain(uint32_t cluster) {
    ...
    while (current < FAT32_EOC && current >= 2) {
        uint32_t next = fat32_get_next_cluster(current);
        fat32_set_fat_entry(current, 0);
        current = next;
    }
}

// Helper: Find entry info (location)
bool fat32_find_entry_info(uint32_t dir_cluster, const char *name, DirEntry_t *out_entry, uint32_t *out_dir_cluster, uint32_t *out_offset) {
    uint32_t cluster_size = bpb.BPB_BytsPerSec * bpb.BPB_SecPerClus;
    uint8_t *cluster_buffer = malloc(cluster_size);
    ...
    while (current_cluster < FAT32_EOC) {
        ...
        for (uint32_t i = 0; i < entries_per_cluster; i++) {
            ...
            if (compare_fat32_name(entries[i].DIR_Name, name)) {
                if(out_entry) memcpy(out_entry, &entries[i], sizeof(DirEntry_t));
                if(out_dir_cluster) *out_dir_cluster = current_cluster;
                if(out_offset) *out_offset = i * sizeof(DirEntry_t);
                free(cluster_buffer);
                return true;
            }
        }
